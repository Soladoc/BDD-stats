...
insert into _academie (code_academie,lib_academie)
  select distinct code_academie as code_academie,libelle_academie
  from _temp_fr_en_etablissements_ep
  WHERE code_academie is not null
  order by code_academie;
--- A compléter
...
insert into _quartier_prioritaire (code_quartier_prioritaire,nom_quartier_prioritaire)
  select distinct qp_a_proximite as qp_a_proximite,nom_du_qp
  from _temp_fr_en_etablissements_ep
  WHERE nom_du_qp is not null
  order by qp_a_proximite;

--- et tout le reste
delete from _type;
insert into _type (code_nature,libelle_nature)
  select distinct code_nature as code_nature,libelle_nature
  from _temp_fr_en_etablissements_ep
...
insert into _etablissement (uai,nom_etablissement,secteur,code_postal,lattitude,longitude,code_insee_de_la_commune,nom_de_la_commune,code_academie,code_nature)
  select distinct _temp_fr_en_etablissements_ep.uai as uai,nom_etablissement,secteur,code_postal,latitude,longitude,code_commune,nom_commune,code_academie,code_nature
  from _temp_fr_en_etablissements_ep join _temp_fr_en_indicateurs_valeur_ajoutee_colleges on _temp_fr_en_indicateurs_valeur_ajoutee_colleges.uai=_temp_fr_en_etablissements_ep.uai
  WHERE _temp_fr_en_etablissements_ep.nom_etablissement is not null
  order by _temp_fr_en_etablissements_ep.uai;
  
delete from _annee;
insert into _annee (annee_scolaire)
  select distinct _temp_fr_en_college_effectif_niveau_sexe_lv.rentree_scolaire as annee_scolaire
  from _temp_fr_en_college_effectif_niveau_sexe_lv
  WHERE rentree_scolaire is not null
  order by rentree_scolaire;
...
insert into _caracteristiques_tout_etablissement (uai,annee_scolaire,effectifs)
  select distinct numero_college as numero_college,rentree_scolaire,nombre_eleves_total
  from _temp_fr_en_college_effectif_niveau_sexe_lv
  WHERE numero_college is not null
  order by numero_college;
...
insert into _caracteristiques_college (uai,annee_scolaire,nbre_eleves_hors_segpa_hors_ulis,nbre_eleves_segpa,nbre_eleves_ulis,ips,ecart_type_de_l_ips,ep)
  select distinct numero_college as numero_college,_temp_fr_en_college_effectif_niveau_sexe_lv.rentree_scolaire,nombre_eleves_total-(nombre_eleves_segpa+nombre_eleves_ulis),nombre_eleves_segpa,nombre_eleves_ulis,ips,ecart_type_de_l_ips,ep_2022_2023
  from _temp_fr_en_college_effectif_niveau_sexe_lv join _temp_fr_en_etablissements_ep on _temp_fr_en_etablissements_ep.uai=_temp_fr_en_college_effectif_niveau_sexe_lv.numero_college
  join _temp_fr_en_ips_colleges_ap2022 on _temp_fr_en_ips_colleges_ap2022.uai=_temp_fr_en_college_effectif_niveau_sexe_lv.numero_college
  WHERE numero_college is not null
  order by numero_college;
...
insert into _est_a_proximite_de (code_quartier_prioritaire,uai)
  select distinct qp_a_proximite as qp_a_proximite,uai
  from _temp_fr_en_etablissements_ep
